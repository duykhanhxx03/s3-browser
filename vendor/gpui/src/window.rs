...
use crate::{
    ...
    AsyncWindowContext, AvailableSpace, Background, BackdropGlass, BorderStyle, Bounds, BoxShadow,
    Capslock,
...
impl Window {
    ...
    /// Paint a pane of backdrop glass: everything already painted this frame
    /// behind `bounds` is captured, blurred by `blur_radius`, tinted with
    /// `tint`, and drawn back masked to the rounded rect. Refraction bends the
    /// sample near the edges, which is what reads as a physical pane rather
    /// than a blurred screenshot.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing, and the capture covers only primitives painted *before* it in
    /// draw order — content painted after (children added later) sits on top
    /// of the glass, not inside it.
    ...
    fn paint_backdrop_glass_inner(
        ...
        let content_mask = self.content_mask();
        let opacity = self.element_opacity();
        self.next_frame.scene.insert_primitive(BackdropGlass {
            order: 0,
