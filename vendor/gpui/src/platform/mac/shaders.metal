...
// --- Backdrop glass -------------------------------------------------------
//
// The pane samples a blurred copy of everything drawn before it. Refraction is
// faked the way every real-time glass fakes it: near the edge, the sample
// coordinate is pushed outward along the SDF gradient, so the rim shows a
// stretched band of what lies just outside the pane — a lens, not a sticker.
...
vertex BackdropGlassVertexOutput backdrop_glass_vertex(
  ...
  float2 unit_vertex = unit_vertices[unit_vertex_id];
  BackdropGlass glass = glasses[glass_id];
  float4 device_position =
...
fragment float4 backdrop_glass_fragment(
  ...
  float2 gradient = float2(
      quad_sdf(pos + float2(1., 0.), glass.bounds, glass.corner_radii) -
          quad_sdf(pos - float2(1., 0.), glass.bounds, glass.corner_radii),
      quad_sdf(pos + float2(0., 1.), glass.bounds, glass.corner_radii) -
          quad_sdf(pos - float2(0., 1.), glass.bounds, glass.corner_radii));
...
// The fullscreen triangle comes out of the vertex id — three vertices, no
// buffers.

struct GlassBlurVertexOutput {
  float4 position [[position]];
  float2 uv;
};

vertex GlassBlurVertexOutput glass_blur_vertex(uint vertex_id [[vertex_id]]) {
  // (0,0) (2,0) (0,2) in uv space covers the unit square once clipped.
  float2 uv = float2((vertex_id << 1) & 2, vertex_id & 2);
  return GlassBlurVertexOutput{
      float4(uv.x * 2. - 1., 1. - uv.y * 2., 0., 1.), uv};
}

fragment float4 glass_blur_fragment(GlassBlurVertexOutput input [[stage_in]],
                                    constant GlassBlurParams *params
                                    [[buffer(0)]],
                                    texture2d<float> source [[texture(0)]]) {
  constexpr sampler blur_sampler(mag_filter::linear, min_filter::linear,
                                 address::clamp_to_edge);
  float sigma = max(params->sigma, 0.5);
  // Steps of two texels, leaning on the linear sampler to average the pair —
  // half the taps of a textbook gaussian for the same footprint.
  int radius = int(ceil(sigma * 1.5));
  // cbindgen renders the Rust arrays as C arrays, not vectors.
  float2 texel = float2(params->direction[0], params->direction[1]) /
                 float2(params->source_size[0], params->source_size[1]);
  float weight_sum = 0.;
  float4 acc = float4(0.);
  for (int offset = -radius; offset <= radius; offset += 2) {
    float weight = exp(-float(offset * offset) / (2. * sigma * sigma));
    acc += source.sample(blur_sampler, input.uv + texel * float(offset)) * weight;
    weight_sum += weight;
  }
